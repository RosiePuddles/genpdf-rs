...
impl FontData {
	...
	/// [`rusttype`]: https://docs.rs/rusttype
	pub fn load(
		path: impl AsRef<path::Path>, builtin: Option<printpdf::BuiltinFont>,
...
impl Font {
	...
	pub fn get_line_height(&self, font_size: u8) -> Mm { self.line_height * f64::from(font_size) }

	/// Returns the glyph height for text with this font and the given font size.
	pub fn glyph_height(&self, font_size: u8) -> Mm { self.glyph_height * f64::from(font_size) }
...
fn from_file(
	dir: impl AsRef<path::Path>, name: &str, style: FontStyle, builtin: Option<Builtin>,
) -> Result<FontData, Error> {
...
pub fn from_files(
	dir: impl AsRef<path::Path>, name: &str, builtin: Option<Builtin>,
