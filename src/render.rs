...
use crate::{
	error::{Context as _, Error, ErrorKind},
	fonts,
	style::{Color, Style},
	Margins, Mm, Position, Size,
};
...
impl Renderer {
	...
	pub fn get_page(&self, idx: usize) -> Option<&Page> { self.pages.get(idx) }

	/// Returns a mutable reference to a page of this document.
	pub fn get_page_mut(&mut self, idx: usize) -> Option<&mut Page> { self.pages.get_mut(idx) }

	/// Returns a mutable reference to the first page of this document.
	pub fn first_page(&self) -> &Page { &self.pages[0] }

	/// Returns the first page of this document.
	pub fn first_page_mut(&mut self) -> &mut Page { &mut self.pages[0] }

	/// Returns the last page of this document.
	pub fn last_page(&self) -> &Page { &self.pages[self.pages.len() - 1] }
	...
	pub fn add_builtin_font(
		&self, builtin: printpdf::BuiltinFont,
...
impl Page {
	fn new(
		page: printpdf::PdfPageReference, layer: printpdf::PdfLayerReference, size: Size,
	) -> Page {
		Page {
			page,
			size,
	...
	pub fn get_layer(&self, idx: usize) -> Option<&Layer> { self.layers.get(idx) }

	/// Returns the first layer of this page.
	pub fn first_layer(&self) -> &Layer { &self.layers[0] }

	/// Returns the last layer of this page.
	pub fn last_layer(&self) -> &Layer { &self.layers[self.layers.len() - 1] }
}
...
impl Layer {
	fn new(layer: printpdf::PdfLayerReference, size: Size) -> Layer { Layer { layer, size } }

	/// Returns a drawable area for this layer.
	pub fn area(&self) -> Area<'_> { Area::new(self, Position::default(), self.size) }
...
impl<'a> Area<'a> {
	...
	pub fn set_size(&mut self, size: impl Into<Size>) { self.size = size.into(); }

	/// Sets the width of this area.
	pub fn set_width(&mut self, width: Mm) { self.size.width = width; }

	/// Sets the height of this area.
	pub fn set_height(&mut self, height: Mm) { self.size.height = height; }
	...
	pub fn print_str<S: AsRef<str>>(
		&self, font_cache: &fonts::FontCache, position: Position, style: Style, s: S,
	) -> Result<bool, Error> {
	...
	pub fn text_section<'f>(
		&self, font_cache: &'f fonts::FontCache, position: Position, style: Style,
	...
	fn layer(&self) -> &printpdf::PdfLayerReference { &self.layer.layer }
}
...
impl<'a, 'f, 'l> TextSection<'a, 'f, 'l> {
	fn new(
		font_cache: &'f fonts::FontCache, area: &'a Area<'l>, position: Position, style: Style,
	...
	fn layer(&self) -> &printpdf::PdfLayerReference { self.area.layer() }
}
