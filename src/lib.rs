...
//!   stores the [`FontCache`][] instance that keeps track of the loaded fonts and can be used to
//!   map a [`Style`][] instance to font data.
...
impl Mm {
	...
	pub fn max(self, other: Mm) -> Mm { Mm(self.0.max(other.0)) }
}

impl From<i8> for Mm {
	fn from(mm: i8) -> Mm { Mm(mm.into()) }
}

impl From<i16> for Mm {
	fn from(mm: i16) -> Mm { Mm(mm.into()) }
}

impl From<i32> for Mm {
	fn from(mm: i32) -> Mm { Mm(mm.into()) }
}

impl From<u8> for Mm {
	fn from(mm: u8) -> Mm { Mm(mm.into()) }
}

impl From<u16> for Mm {
	fn from(mm: u16) -> Mm { Mm(mm.into()) }
}

impl From<u32> for Mm {
	fn from(mm: u32) -> Mm { Mm(mm.into()) }
}

impl From<f32> for Mm {
	fn from(mm: f32) -> Mm { Mm(mm.into()) }
}

impl From<printpdf::Mm> for Mm {
	fn from(mm: printpdf::Mm) -> Mm { Mm(mm.0) }
}
...
impl From<Mm> for printpdf::Mm {
	fn from(mm: Mm) -> printpdf::Mm { printpdf::Mm(mm.0) }
}

impl From<Mm> for printpdf::Pt {
	fn from(mm: Mm) -> printpdf::Pt { printpdf::Mm(mm.0).into() }
}
...
impl From<Position> for printpdf::Point {
	fn from(pos: Position) -> printpdf::Point { printpdf::Point::new(pos.x.into(), pos.y.into()) }
}

impl<X: Into<Mm>, Y: Into<Mm>> From<(X, Y)> for Position {
	fn from(values: (X, Y)) -> Position { Position::new(values.0, values.1) }
}
...
impl<W: Into<Mm>, H: Into<Mm>> From<(W, H)> for Size {
	fn from(values: (W, H)) -> Size { Size::new(values.0, values.1) }
}
...
impl Margins {
	...
	pub fn trbl(
		top: impl Into<Mm>, right: impl Into<Mm>, bottom: impl Into<Mm>, left: impl Into<Mm>,
	) -> Margins {
		Margins {
			top: top.into(),
			right: right.into(),
...
impl<V: Into<Mm>, H: Into<Mm>> From<(V, H)> for Margins {
	fn from(values: (V, H)) -> Margins { Margins::vh(values.0, values.1) }
}

impl<T: Into<Mm>> From<T> for Margins {
	fn from(value: T) -> Margins { Margins::all(value) }
}
...
impl Document {
	...
	pub fn add_font_family(
		&mut self, font_family: fonts::FontFamily<fonts::FontData>,
	...
	pub fn font_cache(&self) -> &fonts::FontCache { &self.context.font_cache }
	...
	pub fn set_font_size(&mut self, font_size: u8) { self.style.set_font_size(font_size); }
	...
	/// [`render`]: #method.render
	/// [`render_to_file`]: #method.render_to_file
	pub fn push<E: Element + 'static>(&mut self, element: E) { self.root.push(element); }
...
pub trait PageDecorator {
	...
	fn decorate_page<'a>(
		&mut self, context: &Context, area: render::Area<'a>, style: style::Style,
...
impl SimplePageDecorator {
	...
	pub fn new() -> SimplePageDecorator { SimplePageDecorator::default() }
...
impl PageDecorator for SimplePageDecorator {
	fn decorate_page<'a>(
		&mut self, context: &Context, mut area: render::Area<'a>, style: style::Style,
	) -> Result<render::Area<'a>, error::Error> {
		self.page += 1;
...
pub trait Element {
	...
	/// [`RenderResult`]: struct.RenderResult.html
	fn render(
		&mut self, context: &Context, area: render::Area<'_>, style: style::Style,
...
impl Context {
	...
	fn new(font_cache: fonts::FontCache) -> Context { Context { font_cache } }
