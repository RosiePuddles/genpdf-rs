...
use crate::{
	...
	wrap, Context, Element, Margins, Mm, Position, RenderResult, Size,
};
...
impl LinearLayout {
	...
	pub fn vertical() -> LinearLayout { LinearLayout::new() }
	...
	fn render_vertical(
		&mut self, context: &Context, mut area: render::Area<'_>, style: Style,
...
impl Element for LinearLayout {
	fn render(
		&mut self, context: &Context, area: render::Area<'_>, style: Style,
	) -> Result<RenderResult, Error> {
...
impl Text {
	...
	pub fn new(text: impl Into<StyledString>) -> Text { Text { text: text.into() } }
}

impl Element for Text {
	fn render(
		&mut self, context: &Context, area: render::Area<'_>, mut style: Style,
	) -> Result<RenderResult, Error> {
		let mut result = RenderResult::default();
...
impl Default for Alignment {
	fn default() -> Alignment { Alignment::Left }
}
...
impl Paragraph {
	...
	pub fn push(&mut self, s: impl Into<StyledString>) { self.text.push(s.into()); }
...
impl PageBreak {
	...
	pub fn new() -> PageBreak { PageBreak::default() }
}

impl Element for PageBreak {
	fn render(
		&mut self, _context: &Context, _area: render::Area<'_>, _style: Style,
	) -> Result<RenderResult, Error> {
		if self.cont {
...
impl Default for UnorderedList {
	fn default() -> UnorderedList { UnorderedList::new() }
}
...
impl OrderedList {
	...
	pub fn new() -> OrderedList { OrderedList::with_start(1) }
...
impl Default for OrderedList {
	fn default() -> OrderedList { OrderedList::new() }
}
...
impl<E: Element> BulletPoint<E> {
	...
	pub fn set_bullet(&mut self, bullet: impl Into<String>) { self.bullet = bullet.into(); }
...
pub trait CellDecorator {
	...
	fn decorate_cell(
		&mut self, column: usize, row: usize, has_more: bool, area: render::Area<'_>, style: Style,
	);
}
...
impl<'a> TableLayoutRow<'a> {
	...
	pub fn push(self) -> Result<(), Error> { self.table_layout.push_row(self.elements) }
}
...
impl TableLayout {
	...
	pub fn row(&mut self) -> TableLayoutRow<'_> { TableLayoutRow::new(self) }
